...
m.reply(`🚫 ¡Oops! Estás en tiempo de descanso. No puedes usar comandos ahora.
📑 Motivo: ${user.messageSpam === 0 ? 'No especificado' : user.messageSpam}.
⚠️ Si crees que hay un error, puedes contactarnos:
📷 Instagram: @leb.manuxs07.
📱 WhatsApp: +5217776120198`);
...
this.reply(m.chat, `No tiene Manucoins`, m)
continue     
}
...
if (m.money)
m.reply(+m.money + ' Manucoins 🐱 𝙐𝙎𝘼𝘿𝙊(𝙎)')  
break
}}} catch (e) {
console.error(e)
} finally {
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '@leb.manuxs07') :
...
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: lenguajeGB.smsAvisoAG().slice(0,-2), body: [wm, '𝗠𝗔𝗡𝗨𝗕𝗢𝗧 ' + gt + ' 👑', '@leb.manuxs07'].getRandom(), thumbnail: gataImg.getRandom(), sourceUrl: [md, nna, nn, nnn, yt, ig, paypal, fb].getRandom() }}}}, tg)
